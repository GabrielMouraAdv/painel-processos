...
export function EditPrazosTceDialog({
  open,
  onOpenChange,
  prazos,
  advogados,
  title = "Editar prazos TCE",
}: Props) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/tce/prazos/${deleteId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Edite ou remova prazos TCE cadastrados. As alteracoes sao salvas
              individualmente.
            </DialogDescription>
          </DialogHeader>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground">
                          Venc. {formatDate(p.dataVencimento)} • {p.diasUteis} dias uteis
