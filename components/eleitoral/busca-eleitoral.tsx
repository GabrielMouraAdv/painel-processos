...
const ORDEM: Resultado["tipo"][] = ["processo", "prazo", "relatorio"];

const TITULOS: Record<Resultado["tipo"], string> = {
  processo: "Processos",
  prazo: "Prazos",
  relatorio: "Relatorios",
};

function Icone({ tipo }: { tipo: Resultado["tipo"] }) {
  const cls = "h-4 w-4 shrink-0";
  switch (tipo) {
    case "processo":
      return <Gavel className={cn(cls, "text-brand-navy")} />;
    case "prazo":
      return <CalendarClock className={cn(cls, "text-red-600")} />;
    case "relatorio":
      return <FileText className={cn(cls, "text-emerald-700")} />;
  }
}

export function BuscaEleitoral({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  // Busca com debounce de 300ms para nao disparar a cada tecla.
  React.useEffect(() => {
    if (!open) return;
    const q = query.trim();
    if (q.length < 2) {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/eleitoral/busca?q=${encodeURIComponent(q)}`,
          { cache: "no-store" },
        );
        if (!res.ok) {
          if (!cancelado) {
  ...
  const curta = query.trim().length < 2;
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Busca no modulo eleitoral"
    >
      <div
      ...
      >
        <div className="flex items-center gap-3 border-b px-4 py-3">
          ...
          <input
            ...
            placeholder="Buscar por numero do processo, parte, relator, prazo..."
          ...
          {curta ? (
            <div className="flex flex-col items-center justify-center px-4 py-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Digite o numero do processo (com ou sem pontos), o nome de uma
                parte, o relator ou uma tarefa.
              </p>
            </div>
